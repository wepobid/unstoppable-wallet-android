...
class AccountViewHolder(override val containerView: View) : RecyclerView.ViewHolder(containerView), LayoutContainer {
    ...
    fun bind(account: WalletConnectViewItem.Account) {
        coinName.text = account.title
    }

    companion object {
        fun create(parent: ViewGroup): AccountViewHolder {
            return AccountViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.view_holder_notification_coin_name, parent, false))
        }
    }
}
...
class SessionViewHolder(
    ...
    fun bind(session: WalletConnectViewItem.Session) {
        ...
        frame.setBackgroundResource(session.position.getBackground())
        icon.loadImage(session.imageUrl)
